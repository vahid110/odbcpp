...
| `SQLBindParameter` | A | Partial | unit, integration | Direction/C/SQL type and length diagnostics, ODBC date/time/timestamp-struct input, temporal target rejection, date/time-to-timestamp and timestamp-to-date/time rules, and declared timestamp fractional precision are covered; character-input precision, input arrays, data-at-execution, and full conversion matrix remain |
