...
TEST_F(PreparedStatementIntegrationTest,
       CharacterTimeInputHonorsLengthsNullAndRecovery) {
    check_temporal_character_lengths(SQL_TYPE_TIME, 0,
        "SELECT (?::time)::text", "12:34:56");
}
...
TEST_F(PreparedStatementIntegrationTest,
    ...
    SQL_TIMESTAMP_STRUCT input{2024, 2, 29, 12, 34, 56, 0};
    check_temporal_character_limits(SQL_C_TYPE_TIMESTAMP, &input, sizeof(input),
                                    "2024-02-29 12:34:56");
    input.fraction = 120000000u;
    check_temporal_character_limits(SQL_C_TYPE_TIMESTAMP, &input, sizeof(input),
                                    "2024-02-29 12:34:56.12");
    input.fraction = 123456789u;
    check_temporal_character_limits(SQL_C_TYPE_TIMESTAMP, &input, sizeof(input),
...
TEST_F(PreparedStatementIntegrationTest,
       NumericStructureIntegerParameterTruncatesFractionAndRecovers) {
    ASSERT_EQ(SQL_SUCCESS, SQLPrepare(hstmt,
        (SQLCHAR*)"SELECT ?::integer", SQL_NTS));
    SQL_NUMERIC_STRUCT numeric{};
    numeric.sign = 1;
    numeric.val[0] = 0x39;
    numeric.val[1] = 0x30;  // 12345, or 123.45 at APD scale 2.
    SQLLEN indicator = 0;
    ASSERT_EQ(SQL_SUCCESS, SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT,
        SQL_C_NUMERIC, SQL_INTEGER, 12, 0, &numeric, 0, &indicator));
    SQLHDESC apd = SQL_NULL_HDESC;
    ASSERT_EQ(SQL_SUCCESS, SQLGetStmtAttr(hstmt, SQL_ATTR_APP_PARAM_DESC,
        &apd, 0, nullptr));
    const auto number = [](SQLLEN field) {
    ...
    ASSERT_EQ(SQL_SUCCESS, SQLSetDescField(apd, 1,
        SQL_DESC_PRECISION, number(12), 0));
    ASSERT_EQ(SQL_SUCCESS, SQLSetDescField(apd, 1,
        SQL_DESC_SCALE, number(2), 0));
    ASSERT_EQ(SQL_SUCCESS, SQLSetDescField(apd, 1,
        SQL_DESC_DATA_PTR, &numeric, 0));
    ...
    numeric = {};
    numeric.sign = 0;
    numeric.val[0] = 75;  // -0.75 truncates toward zero.
    expect_value("0");

    ASSERT_EQ(SQL_SUCCESS, SQLSetDescField(apd, 1,
        SQL_DESC_SCALE, number(0), 0));
    ASSERT_EQ(SQL_SUCCESS, SQLSetDescField(apd, 1,
        SQL_DESC_DATA_PTR, &numeric, 0));
    numeric = {};
    numeric.sign = 1;
    numeric.val[3] = 0x80;  // 2147483648.
    SQLCHAR state[6]{};
    EXPECT_EQ(SQL_ERROR, SQLExecute(hstmt));
    ASSERT_EQ(SQL_SUCCESS, SQLGetDiagRec(SQL_HANDLE_STMT, hstmt, 1,
        state, nullptr, nullptr, 0, nullptr));
...
TEST_F(PreparedStatementIntegrationTest,
       NumericStructureSmallintAndBigintBoundaries) {
    const auto check_boundary = [&](const char* statement,
                                    SQLSMALLINT sql_type,
                                    SQLULEN precision,
                                    SQL_NUMERIC_STRUCT numeric,
                                    const char* expected) {
        ASSERT_EQ(SQL_SUCCESS, SQLPrepare(hstmt,
            reinterpret_cast<SQLCHAR*>(const_cast<char*>(statement)), SQL_NTS));
        ASSERT_EQ(SQL_SUCCESS, SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT,
            SQL_C_NUMERIC, sql_type, precision, 0, &numeric, 0, nullptr));
        ASSERT_EQ(SQL_SUCCESS, SQLExecute(hstmt));
        ASSERT_EQ(SQL_SUCCESS, SQLFetch(hstmt));
        char result[32]{};
        ASSERT_EQ(SQL_SUCCESS, SQLGetData(hstmt, 1, SQL_C_CHAR,
        ...
        numeric.sign = 1;
        if (sql_type == SQL_SMALLINT) {
            numeric.val[0] = 0x00;
            numeric.val[1] = 0x80;  // 32768.
        } else {
            numeric.val[7] = 0x80;  // 9223372036854775808.
        }
        SQLCHAR state[6]{};
        EXPECT_EQ(SQL_ERROR, SQLExecute(hstmt));
        ASSERT_EQ(SQL_SUCCESS, SQLGetDiagRec(SQL_HANDLE_STMT, hstmt, 1,
    ...
    SQL_NUMERIC_STRUCT small_numeric{};
    small_numeric.sign = 0;
    small_numeric.val[1] = 0x80;  // -32768.
    check_boundary("SELECT ?::smallint", SQL_SMALLINT, 5, small_numeric,
        "-32768");

    SQL_NUMERIC_STRUCT large_numeric{};
    large_numeric.sign = 0;
...
TEST_F(PreparedStatementIntegrationTest,
       NumericStructureBitParameterClassifiesFractionsAndRange) {
    ASSERT_EQ(SQL_SUCCESS, SQLPrepare(hstmt,
        (SQLCHAR*)"SELECT ?::boolean", SQL_NTS));
    SQL_NUMERIC_STRUCT numeric{};
    numeric.sign = 1;
    SQLLEN indicator = 0;
    ASSERT_EQ(SQL_SUCCESS, SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT,
        SQL_C_NUMERIC, SQL_BIT, 1, 0, &numeric, 0, &indicator));
    SQLHDESC apd = SQL_NULL_HDESC;
    ASSERT_EQ(SQL_SUCCESS, SQLGetStmtAttr(hstmt, SQL_ATTR_APP_PARAM_DESC,
        &apd, 0, nullptr));
    const auto number = [](SQLLEN field) {
    ...
    const auto expect_bit = [&](SQLCHAR expected) {
        ...
        SQLCHAR actual = 9;
        ASSERT_EQ(SQL_SUCCESS, SQLGetData(hstmt, 1, SQL_C_BIT,
            &actual, sizeof(actual), nullptr));
        EXPECT_EQ(expected, actual);
        ASSERT_EQ(SQL_SUCCESS, SQLCloseCursor(hstmt));
    };
    expect_bit(0);
    numeric.val[0] = 10;  // 1.0.
    expect_bit(1);

    const auto expect_failure = [&](SQLCHAR magnitude, SQLCHAR sign,
                                    const char* expected_state) {
        numeric.val[0] = magnitude;
        numeric.sign = sign;
        SQLCHAR state[6]{};
        EXPECT_EQ(SQL_ERROR, SQLExecute(hstmt));
        ASSERT_EQ(SQL_SUCCESS, SQLGetDiagRec(SQL_HANDLE_STMT, hstmt, 1,
    ...
    expect_failure(5, 1, "22001");   // 0.5.
    expect_failure(15, 1, "22001");  // 1.5.
    expect_failure(5, 0, "22003");   // -0.5.
    expect_failure(20, 1, "22003");  // 2.0.
    ...
    SQLCHAR untouched = 9;
    SQLLEN length = 91;
    EXPECT_EQ(SQL_SUCCESS, SQLGetData(hstmt, 1, SQL_C_BIT,
        &untouched, sizeof(untouched), &length));
    EXPECT_EQ(SQL_NULL_DATA, length);
    EXPECT_EQ(9, untouched);
}
...
TEST_F(PreparedStatementIntegrationTest,
       CharacterToDecimalHonorsDeclaredWholeDigitsAfterDescribe) {
    ASSERT_EQ(SQL_SUCCESS, SQLPrepare(hstmt,
        (SQLCHAR*)"SELECT ?::numeric", SQL_NTS));
    char value[32] = "  +00100.0  ";
    SQLLEN value_length = SQL_NTS;
    ASSERT_EQ(SQL_SUCCESS, SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT,
        SQL_C_CHAR, SQL_DECIMAL, 3, 1, value, sizeof(value), &value_length));
    SQLSMALLINT server_type = 0;
    ASSERT_EQ(SQL_SUCCESS, SQLDescribeParam(hstmt, 1,
        &server_type, nullptr, nullptr, nullptr));
    EXPECT_EQ(SQL_NUMERIC, server_type);
...
TEST_F(PreparedStatementIntegrationTest,
       WideCharacterToNumericHonorsDeclaredWholeDigits) {
    ASSERT_EQ(SQL_SUCCESS, SQLPrepare(hstmt,
        (SQLCHAR*)"SELECT ?::numeric", SQL_NTS));
    SQLWCHAR value[]{'1', '0', '0', '.', '0', 0};
    SQLLEN value_length = SQL_NTS;
    ASSERT_EQ(SQL_SUCCESS, SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT,
        SQL_C_WCHAR, SQL_NUMERIC, 3, 1, value, sizeof(value), &value_length));
    SQLSMALLINT server_type = 0;
    ASSERT_EQ(SQL_SUCCESS, SQLDescribeParam(hstmt, 1,
        &server_type, nullptr, nullptr, nullptr));
    ASSERT_EQ(SQL_ERROR, SQLExecute(hstmt));
...
TEST_F(PreparedStatementIntegrationTest,
       CharacterToNumericRejectsSignificantFractionalLoss) {
    ASSERT_EQ(SQL_SUCCESS, SQLPrepare(hstmt,
        (SQLCHAR*)"SELECT ?::numeric", SQL_NTS));
    char value[32] = "1.234";
    SQLLEN value_length = SQL_NTS;
    ASSERT_EQ(SQL_SUCCESS, SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT,
        SQL_C_CHAR, SQL_NUMERIC, 4, 2, value, sizeof(value), &value_length));
    SQLSMALLINT server_type = 0;
    ASSERT_EQ(SQL_SUCCESS, SQLDescribeParam(hstmt, 1,
        &server_type, nullptr, nullptr, nullptr));
    EXPECT_EQ(SQL_NUMERIC, server_type);
    const auto expect_loss = [&] {
        ASSERT_EQ(SQL_ERROR, SQLExecute(hstmt));
        SQLCHAR state[6]{};
        ASSERT_EQ(SQL_SUCCESS, SQLGetDiagRec(SQL_HANDLE_STMT, hstmt, 1,
            state, nullptr, nullptr, 0, nullptr));
    ...
    SQLWCHAR wide_value[]{'1', '.', '2', '3', '4', 0};
    ASSERT_EQ(SQL_SUCCESS, SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT,
        SQL_C_WCHAR, SQL_NUMERIC, 4, 2, wide_value, sizeof(wide_value),
        &value_length));
    ASSERT_EQ(SQL_SUCCESS, SQLDescribeParam(hstmt, 1,
        &server_type, nullptr, nullptr, nullptr));
    expect_loss();
